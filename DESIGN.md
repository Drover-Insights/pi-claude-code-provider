...
Literal at signs are JSON Unicode-escaped because Claude expands `@path` syntax. Only provider-generated, validated images remain attachment references. Their dynamic reference list follows the append-stable transcript blocks so adding an image does not invalidate the reusable history prefix. Only images from the current user turn, meaning the latest user message and the tool results after it, are attached. Any attachment makes Claude Code narrate its read ahead of the transcript and costs that request its cache reuse, so re-attaching an old image would keep every later request uncached. Earlier images keep an identical transcript record, which keeps history append-stable, but are not re-attached and do not count toward the per-request image limits.
...
The CLI prepends a billing header block, carrying its own exact version, ahead of everything else in the system array, where it is never marked for caching. Upgrading Claude Code therefore invalidates every cached prefix, which is expected but worth knowing before reading a cold start as a regression. Observed on 2.1.261 through 2.1.269.

In print mode the CLI also prepends its own identity line, `You are a Claude agent, built on Anthropic's Claude Agent SDK.`, directly to the system prompt supplied through `--system-prompt-file`, with no separating newline. The branch is selected by non-interactivity rather than by the Agent SDK, so it applies on this project's documented path; `--append-system-prompt` only exchanges it for a different identity line. Nothing is appended after the supplied prompt. Observed on 2.1.261 through 2.1.269.

The CLI also injects a block of its own environment context, which from 2.1.268 names the private request directory the provider spawns it in. Neither `--setting-sources ""`, the pinned settings object, nor `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` suppresses it, so a request carries context the provider never supplied and cannot remove. Where it lands is not stable across releases or model families: through 2.1.267 it was a date-only reminder at the head of the first user message, while from 2.1.268 the Claude 5 aliases receive it as a trailing `role: "system"` message after the transcript and Haiku 4.5 still receives it at the head. That difference is what decides whether prompt caching works, as the compatibility section below explains.
