...
The default extension preserves the ambient single-provider contract when `PI_CLAUDE_CODE_PROVIDER_CONFIG` is unset. The named instance factory is an explicit alternative for wrappers that need more than one official Claude Code subscription in one Pi session. When that variable is set, the default extension reads the same configuration object from one bounded private file before initialization. It requires a canonical absolute non-symlink path, a stable regular file owned by the current user, POSIX mode `0600`, valid UTF-8 JSON, and an exact descriptor field allowlist. It reports configuration failures without the file path. The existing Claude process environment allowlist omits the variable, so the private control path does not cross into Claude Code.
