...
The CLI also injects a block of its own environment context. From 2.1.268 it states Claude's process working directory as the model's primary working directory and says whether that directory is a git repository. Neither `--setting-sources ""`, the pinned settings object, nor `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` suppresses it. `--bare` does, but it never reads a subscription login. A request therefore carries context the provider never supplied and cannot remove.

Where the block lands is not stable across releases or model families. Through 2.1.267 it was a date-only reminder at the head of the first user message. From 2.1.268 the Claude 5 aliases receive it as a trailing `role: "system"` message after the transcript, the last thing the model reads, while Haiku 4.5 still receives it at the head. That difference decides whether prompt caching works, as the compatibility section below explains.
...
- **What still runs.** With that setting, 2.1.261 and 2.1.270 were observed running read-only `git ls-files` and `git remote` probes and an `rg --files` count of the project, and reading the project's `.claude/settings*.json`. None of the project's customizations, hooks or MCP servers took effect, and nothing was written to the project.
