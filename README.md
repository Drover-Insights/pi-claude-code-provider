...
The provider accepts Pi 0.85.1's top-level prompt and tools and Pi 0.86.0/0.86.1's transcript system messages. On the newer contract it uses Pi's replay helpers to recover the current prompt and active tools, including later section and tool changes, before sending the request to Claude Code. The [compatibility baseline](DEVELOPING.md#compatibility-baseline) remains 0.85.1 until the full release gate runs on a newer build; 0.86.1 has only a capped Sonnet-low tool round-trip check on npm and standalone Pi.
