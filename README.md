...
- **A subagent request fails saying its session "was never started through this provider":** several Pi sessions are live in one process and the request neither belongs to one of them nor states its own working directory, so there is no directory that is certainly right. Requests that do name one, including subagents working in their own directory or worktree, run there. If one reports a conflict with the session it belongs to, that request was routed to the wrong session and was refused rather than run in the wrong tree.
