...
- **Another extension fails with `No API provider registered for api: pi-claude-code-provider-headless`, or Pi exits when one runs:** an extension running its own agent loop on a provider model reaches Pi-AI's API registry rather than Pi's model runtime. The provider now serves both, so update to this version; such calls are ordinary requests, and the calling extension still runs its own tools.
...
- **A request fails with `working_directory`:** Claude runs in the working directory Pi states for that request, and that directory is not absolute, cannot be read (usually because it no longer exists), is not a directory, or no Pi session has started. The provider never runs Claude somewhere else instead. Restart Pi from an existing directory.
- **A request fails saying its session "was never started through this provider":** several Pi sessions are live in one process — usually another extension running its own agent loop under a session id of its own — and the request neither belongs to one of those sessions nor states its own working directory, so there is no directory that is certainly right. Requests that do name one, including a caller working in its own directory or worktree, run there. If one reports a conflict with the session it belongs to, that request was routed to the wrong session and was refused rather than run in the wrong tree.
