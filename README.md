...
This release accepts Pi 0.85.1's provider context, with the system prompt and active tools at the top level. A later Pi checkout moves them into transcript system messages; requests containing those messages fail with `content_shape` until this provider supports their replay rules. See [DEVELOPING.md](DEVELOPING.md#pi-provider-context-after-0851) before adopting a newer Pi release.
...
- **A request fails with `working_directory`:** No Pi session has started; the selected cwd is unusable; or a tool-bearing request has no registered session and no recognized cwd declaration. The provider refuses that last case even with one registered session. The caller should supply its cwd in Pi's prompt format (`Current working directory: <absolute path>` at the end on Pi 0.85.1). `PI_CLAUDE_CODE_PROVIDER_BORROW_SOLE_DIRECTORY=on` restores the older sole-session borrow if you accept that it may run Claude in the wrong tree. A registered session whose prompt names another cwd is also refused.
- **pi-subagents watchdog or permission helper fails with `working_directory`:** Ordinary child turns still carry Pi's generated cwd and work. pi-subagents' separate direct Agents do not all pass a cwd this provider recognizes. Its extra child watchdog is off by default; when enabled, its read-only review may fail. Main watchdog reviews and decision helpers can also fail when they use this provider, and a failed child permission arbiter denies the protected tool call. Ask pi-subagents to pass each helper's cwd in Pi's prompt form (and its session ID where the child is registered), or use the compatibility setting above with its stated risk. No pi-subagents-specific prompt parser is installed here.
- **A request fails with `content_shape` mentioning transcript system messages:** The installed Pi uses a provider input shape newer than this release supports. Use Pi 0.85.1 until a transcript adapter has been implemented and verified for the newer release.
