...
- Historical images remain attached after Claude replies, so Claude can re-inspect them later in the same Pi context. The provider stores each image at a stable private path for the Pi session; repeated requests can then reuse Claude's cached transcript prefix instead of losing caching to changing attachment paths. New images or changed context can still cause a cold write. Historical images continue to count toward the existing 20-image and byte limits. The context protocol remains `pi-claude-code-provider-context-v4`; the updated transcript instruction causes one cold prefix after upgrading from an earlier 0.3.0 development build.
...
- The paid release gate adds `npm run test:paid:cache-haiku` and Sonnet and Haiku image-cache stages. Every cache stage requires each reuse turn to read at least 80% from cache and write less than a quarter of turn 1's cache write; the image stages also verify reinspection of an image attached only on turn 1.
