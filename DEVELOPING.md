...
Claude Code 2.1.268 then moved the final `cache_control` marker off the transcript entirely, which no setting restores, so `providerArgs` marks the last history block itself with `ttl: "1h"`. The TTL is load-bearing rather than a lifetime preference: the API requires breakpoints in longest-TTL-first order and Claude Code places one of its own 1h markers after this one, so a shorter TTL is rejected outright. Four breakpoints is the documented maximum, and every configuration measured on an affected build sits at exactly four, so a fifth from any source turns a slow request into a failed one. Count them before changing anything here. The verified baseline is no longer held back by this gate, but moving it still requires a full release-gate run on the new build.

Two cases stay outside the reuse this buys, and both are upstream. Haiku 4.5 receives the environment content ahead of the transcript instead of after it, so no marker placement reaches it; it is deliberately not gated, and the cache stage stays on Sonnet, where the breakpoint actually moved. Requests carrying generated image attachments have never cached through this provider on any build, because the CLI narrates its own attachment read, including the private directory path, ahead of the transcript.
