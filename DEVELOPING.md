...
Claude Code 2.1.268 then moved the final `cache_control` marker off the transcript entirely, which no setting restores, so `providerArgs` marks the last history block itself with `ttl: "1h"`. The TTL is load-bearing rather than a lifetime preference: the API requires breakpoints in longest-TTL-first order and Claude Code places one of its own 1h markers after this one, so a shorter TTL is rejected outright. Four breakpoints is the documented maximum, and every Claude 5 alias measured on an affected build sits at exactly four, Haiku at three, so a fifth from any source turns a slow request into a failed one. Count them before changing anything here. The verified baseline is no longer held back by this gate, but moving it still requires a full release-gate run on the new build.

`npm run capture:claude-breakpoints` reproduces all of this without spending quota. It builds a request from the provider's own `providerArgs` and `buildClaudeEnvironment`, captures it against a loopback server with a dummy token, and prints the breakpoint table, the first block that differs between two captures in different private directories, and a verdict; it exits non-zero unless the shape is healthy. `--claude` selects a build, `--model` an alias, `--strip-marker` gives the control arm without editing `src/`, and `--images` and `--no-tools` vary the payload. Use it before trusting any claim in this section against a new release.

Measured reuse by alias, on 2.1.269 at low effort. Every Claude 5 alias recovers; only Haiku does not:
...
| `haiku` | 0% | Two independent causes: the environment content arrives ahead of the transcript, and Haiku 4.5 needs 4096 tokens before anything caches at all, the highest minimum of any current model. |

**A single zero reading is not evidence.** While measuring the table above, four consecutive Opus runs read 0% and were nearly written up as an account-level defect. Requests captured from the passing and failing harnesses were byte-identical once the per-request directory and metadata were normalized, and the same probe later read 99% unchanged. Nothing was ever found to distinguish them. Before concluding that a model or a serialization change has broken caching, repeat the measurement, and diff the wire requests with `npm run capture:claude-breakpoints` rather than trusting one run.

`PI_CLAUDE_CODE_PROVIDER_CACHE_MODEL` points the cache stage at another alias, defaulting to `sonnet:low`. Expect the third turn's wording assertion to be less reliable on other aliases; Opus returned empty text there. For a measurement that does not depend on the model's exact reply, use a two-request usage probe instead.
