...
| --- | --- | --- |
| Extension startup, private configured-instance files, configured account instances, ordered failover, session lifetime, and session working directory | `extensions/index.ts` (manifest entry), `extensions/pi-claude-code-provider.ts`, `src/configured-instance-file.ts` (private file boundary), `src/configured-instances.ts` (descriptor and root validation), `src/failover.ts` | `extension.test.js` |
