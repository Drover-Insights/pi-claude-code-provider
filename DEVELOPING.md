...
A request that attaches an image does not cache on any build, because the CLI narrates its own attachment read, including the private directory path, ahead of the transcript. `prepareRequest` therefore attaches only the current user turn's images. When changing serialization, keep earlier images out of the attachment list and their transcript records unchanged.
